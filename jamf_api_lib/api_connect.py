...
"""Functions that help connect to the API"""
...
def write_token_to_json_file(url, jamf_user, data, jamfupload_token_file=""):
    ...
    if not jamfupload_token_file:
        jamfupload_token_file = init_temp_file(prefix="jamf_upload_token_")
    with open(jamfupload_token_file, "w", encoding="utf-8") as fp:
        json.dump(data, fp)
...
def make_tmp_dir(tmp_dir="/tmp/jamf_upload_"):
    ...
    base_dir, directory = tmp_dir.rsplit("/", 1)
    jamfupload_tmp_dir = tempfile.mkdtemp(prefix=directory, dir=base_dir)
    return jamfupload_tmp_dir


def init_temp_file(prefix="jamf_upload_", suffix=None, directory=None, text=True):
    """dump some text to a temporary file"""
    return tempfile.mkstemp(
        prefix=prefix,
        suffix=suffix,
        dir=make_tmp_dir() if directory is None else directory,
        text=text,
    )[1]
...
def get_credentials(prefs_file):
    ...
    if list(filter(prefs_file.endswith, read_as_json)):
        with open(prefs_file, "r", encoding="utf-8") as js:
            prefs = json.load(js)
...
def get_uapi_token(jamf_url, jamf_user, enc_creds="", verbosity=""):
    ...
    if enc_creds:
        ...
        r = curl.request(method="POST", url=url, auth=enc_creds, verbosity=verbosity)
